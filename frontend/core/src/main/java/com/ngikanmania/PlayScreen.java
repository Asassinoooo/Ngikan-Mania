...
public class PlayScreen implements Screen, FishObserver {
    ...
    private final float playerWidth = 32f;
    private final float playerHeight = 64f;
    ...
    private final com.badlogic.gdx.utils.Pool<PlaceholderFish> placeholderFishPool;
    ...
    public PlayScreen() {
        ...
        playerTex = assets.placeholderTexture;
        ...
        placeholderFishPool = new com.badlogic.gdx.utils.Pool<PlaceholderFish>() {
            @Override
            protected PlaceholderFish newObject() {
                return new PlaceholderFish();
            }
        };
    ...
    public void render(float delta) {
        ...
        for (int i = activeFishes.size - 1; i >= 0; i--) {
            ...
            if (!isAlive) {
                ...
                } else if (fish instanceof PlaceholderFish) {
                    placeholderFishPool.free((PlaceholderFish) fish);
                }
            }
        }
        ...
        float t = 4f;
        // Left
        assets.batch.draw(playerTex, 0, 0, t, WORLD_HEIGHT); 
        // Right
        assets.batch.draw(playerTex, WORLD_WIDTH - t, 0, t, WORLD_HEIGHT); 
        // Top
        assets.batch.draw(playerTex, 0, WORLD_HEIGHT - t, WORLD_WIDTH, t); 
        // Bottom
        assets.batch.draw(playerTex, 0, 0, WORLD_WIDTH, t); 
        ...
        for (Bullet b : activeBullets) {
            b.draw(assets.batch, playerTex);
        }
        
        // Render the placeholder sprite rotating around its center to face the cursor
        assets.batch.draw(playerTex, 
            playerPos.x - playerWidth / 2f, playerPos.y - playerHeight / 2f, 
            playerWidth / 2f, playerHeight / 2f, 
            playerWidth, playerHeight, 
            1f, 1f, 
            renderAngle, 
            0, 0, playerTex.getWidth(), playerTex.getHeight(), 
            false, false);
    ...
    private void spawnFish() {
        ...
        BaseFish leader = (type == FishType.SMALL) ? clownfishPool.obtain() : placeholderFishPool.obtain();
        leader.init(startX, startY, 0f, 0f, playerTex, strategy, type); 
        ...
        for (int i = 1; i < swarmCount; i++) {
            ...
            BaseFish follower = (type == FishType.SMALL) ? clownfishPool.obtain() : placeholderFishPool.obtain();
            // Supply existing velocity directly via init() to freeze swarm trajectory completely
            follower.init(startX + offset.x, startY + offset.y, leaderVx, leaderVy, playerTex, strategy, type); 
